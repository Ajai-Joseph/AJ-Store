...
class BuyChatScreen extends StatefulWidget {
  ...
  @override
  State<BuyChatScreen> createState() => _BuyChatScreenState();
}

class _BuyChatScreenState extends State<BuyChatScreen> with SingleTickerProviderStateMixin {
  ...
  void sendMessage() async {
    ...
    try {
      ...
      final productDoc = await firestore.collection("Products").doc(widget.productId).get();
      if (productDoc.exists) {
        List buyers = productDoc.data()!['Buyers'] ?? [];
        buyers.add(auth.currentUser!.uid);
        Set buyersSet = buyers.toSet();
        await firestore
            .collection("Products")
            .doc(widget.productId)
            .update({'Buyers': buyersSet.toList()});
      }
